...
const About = ({ halaman }) => {
	return (
		<>
			<BodyMain breadcrumb={[{ name: halaman, type: "current" }]}>
				<h1 className="page-title">{halaman}</h1>
				{halaman == "DMCA" ? <Dmca /> : <Privacy />}
			</BodyMain>
		</>
	)
}
...
const Dmca = () => {
	return (
		<>
			<p className="mb-4">
				Menime is an online website service provider like defined in Digital
				Millenium Copyright Act. We respect the copyright laws and will protect
				the right of every copyright owner seriously. If you are the owner of
				any content showed on Menime and you dont want to allow us to use the
				content, then you are very able to tell us by email to dmca@menime.com
				so that we can identify and take necessary action. We cannot take any
				action if you dont give us any information about it, so please send us
				an email with the details like:
			</p>
			<ul className="pl-8 mb-4 list-disc">
				<li>
					Specification about copyright of content which claimed to be
					infringed,
				</li>
				<li>
					If you claimed about infringement from some copyright works in one
					email, please write the list about it in detail including website urls
					contain exact content that claimed to be infringing,
				</li>
				<li>
					Give us information about your name, phone, office address and email
					address also to allow us contact you if neccessary,
				</li>
				<li>
					We really expect that the sender would be real copyright owner and not
					the third party or agents,
				</li>
				<li>
					Information written must be accurate and under the law of
					counterfeiting.
				</li>
			</ul>
		</>
...
const Privacy = () => {
	return (
		<>
			...
			<p>
				Menime doesn’t host any content on it own server and just linking to or
				embedding content that was uploaded to popular Online Video hosting
				sites like dailymotion.com, Youtube.com, Google Drive, ZShare.net,
				cloudy, netu.tv and such sites.
			</p>
			<p>
				All trademarks, Videos, trade names, service marks, copyrighted work,
				logos referenced herein belong to their respective owners/companies.
				Menime is not responsible for what other people upload to 3rd party
				sites. We urge all copyright owners, to recognize that the links
				contained within this site are located somewhere else on the web or
				video embedded are from other various site like included above!.
			</p>
			<p>
				If you have any legal issues please contact appropriate media file
				owners / hosters. This privacy policy is intended to inform you about
				the types of information gathered by us when you visit Menime.
			</p>
